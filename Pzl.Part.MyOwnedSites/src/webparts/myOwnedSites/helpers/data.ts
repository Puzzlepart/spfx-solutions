...
export const getOwnedGroupSites = async (context: WebPartContext, loadedPages: ISiteListPage[], nextPage: string | undefined, hash: string | undefined): Promise<ISiteResponse> => {
    const graphClient = await context.msGraphClientFactory.getClient('3') as MSGraphClientV3
    const filter = hash && hash !== '' ? `&$filter=startsWith(displayName, '${hash}')` : '';
    const url = nextPage ?? `https://graph.microsoft.com/v1.0/me/ownedObjects/microsoft.graph.group?$select=mailNickname,id,displayName,description${filter}&$count=true&$top=10`;
    const res = await graphClient.api(url).headers({"ConsistencyLevel": "eventual"}).get();
...
export const getCreatedSites = async (client: SPFI, user: string, loadedPages: ISiteListPage[], startRow: number, hash: string): Promise<ISiteResponse> => {
    const previousPage = loadedPages[loadedPages.length - 1]?.page || 0;
    const page: ISiteListPage = { page: previousPage + 1, sites: [] };

    const queryText = hash && hash !== '' ? hash : '*';
    const q = SearchQueryBuilder().text(`${queryText} contentclass:sts_site People:${user}`)
