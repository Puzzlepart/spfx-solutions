...
export default class EnableTeamsApplicationCustomizer
  ...
  public onInit(): Promise<void> {
    ...
    if (isSiteAdmin) {
      ...
      if (typeof this.properties.payload !== 'undefined') {
        payload = this.properties.payload;
      }
      this.DoWork(autoCreate, shouldRedirect, payload);
    }
    return Promise.resolve();
  }

  private async DoWork(autoCreate: boolean, shouldRedirect: boolean, payload: any) {
    let failed = false;
    let teamsUri;
    let hasTeam = false;
    ...
    try {
      if (!hasTeam && autoCreate) {
        ...
        dialog.show();
        teamsUri = await Functions.CreateTeam(this.context.graphHttpClient, groupId, this.context.pageContext.site.absoluteUrl, payload);
        hasTeam = true;
      }
    } catch (error) {
      failed = true;
    ...
    if (!this._topPlaceholder && !autoCreate) {
      ...
      let buttonProps = { graphClient: this.context.graphHttpClient, groupId: groupId, siteUrl: this.context.pageContext.site.absoluteUrl, componentId: this.componentId, shouldRedirect: shouldRedirect, payload: payload };
